...
@test full(CHOLMOD.Sparse("tmp.mtx")) == [1 0 0;0 1 0.5;0 0.5 1]
run(`rm tmp.mtx`)
writedlm("tmp.mtx", ["%%MatrixMarket matrix coordinate complex Hermitian","3 3 4","1 1 1.0 0.0","2 2 1.0 0.0","3 2 0.5 0.5","3 3 1.0 0.0"])
@test full(CHOLMOD.Sparse("tmp.mtx")) == [1 0 0;0 1 0.5-0.5im;0 0.5+0.5im 1]
run(`rm tmp.mtx`)
...
pa = pointer_to_array(convert(Ptr{Uint32}, p), 8*2 + 6, false)
pa[8*2 + 4] = CHOLMOD.SINGLE
...
# test that Sparse(Ptr) works for SuiteSparse_long
...
for elty in (Float64, Complex{Float64})
    ...
    @test_approx_eq det(F) det(full(A1pd))
    @test isposdef(A1pd)
    @test !isposdef(A1)
    if elty <: Real
